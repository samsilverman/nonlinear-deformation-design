...
from nonlinear_deformation_design import set_seed, load_data, split_indices, get_parameters_processor, get_performance_processor, TNN, print_model, WeightedMSELoss, load_checkpoint, save_checkpoint
import numpy as np
import torch
...
def build_parser() -> ArgumentParser:
    ...
    """
    parser = ArgumentParser(description='Train forward model.', formatter_class=ArgumentDefaultsHelpFormatter)
    ...
    parser.add_argument('--out-dir', type=Path, default=Path('results'), help='Output directory for the saved forward model.')
...
def main() -> None:
    """Train forward model from the command line.
    ...
    checkpoint_file = args.out_dir / 'forward-checkpoint.pt'
    model_file = args.out_dir / 'forward-model.pt'
    losses_file = args.out_dir / 'forward-losses.png'
